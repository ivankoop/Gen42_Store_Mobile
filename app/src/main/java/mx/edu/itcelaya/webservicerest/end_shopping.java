...
public class end_shopping extends AppCompatActivity {
    String jsonResult;
    String url = "http://gen-42-shop.000webhostapp.com/api/orders";
...
//        ProductList = (ArrayList<abs_Products>) getIntent().getSerializableExtra("productList");
  //      car = (ArrayList<abs_car_buys>) getIntent().getSerializableExtra("carList");
    ...
    private void loadAddress(){
        ...
        String SampleURL;
        if(!id_customer_paid.getText().toString().equalsIgnoreCase(""))
            SampleURL = "http://gen-42-shop.000webhostapp.com/api/addresses&output_format=JSON&display=full&filter%5Bid_customer%5D="+id_customer_paid.getText().toString();
        else
            SampleURL = "http://gen-42-shop.000webhostapp.com/api/addresses&output_format=JSON&display=full";
        Log.e("url", SampleURL);
        try {
...
*/
        //Toast.makeText(getApplicationContext(), "cart " +_ID_cart,
          //      Toast.LENGTH_LONG).show();
        WebServiceTask wst = new WebServiceTask( this, "Adding...", prestashop_key);
